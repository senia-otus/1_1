...
class PostServiceImpl(
  ...
  override def findAllPosts(userId: Long): Seq[PostView] =
    postDao.allPosts().value.get match {
      case Success(value) => value.filter(_.authorId == userId).flatMap(mapToView)
      case _              => Seq.empty[PostView]
    }

  override def findPost(id: Long): Option[PostDetailsView] =
    postDao
      .getPost(id)
      .flatMap { post =>
        for {
  ...
  override def searchPosts(mask: String): Seq[PostView] =
    postDao
      .allPosts()
      .zip(tagDao.search(mask))
      .map {
        case (posts: Seq[Post], tags: Seq[Tag]) =>
          posts.filter(_.title == mask).map(p => mapToView(p)).map(_.get) ++ searchPostsByTagMask(
            posts,
            tags
          ) ++ Nil
      }
